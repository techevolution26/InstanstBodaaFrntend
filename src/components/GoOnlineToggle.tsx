...
export default function GoOnlineToggle() {
  ...
  // Sending the current coords + online flag to the API
  ...
  return (
    <button
      onClick={toggle}
      disabled={loading}
      className={`px-4 py-2 rounded ${online ? 'bg-red-600 text-white' : 'bg-green-600 text-white'
        }`}
    >
      {online ? (
        <SignalSlashIcon className="inline-block w-5 h-5 mr-2" />
      ) : (
        <SignalIcon className="inline-block w-5 h-5 mr-2" />
      )}
      {loading
        ? 'Updating…'
        : online
