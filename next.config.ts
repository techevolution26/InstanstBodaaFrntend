...
module.exports = {
  async rewrites() {
    ...
    if (apiBase) {
      ...
      rewrites.unshift(
        { source: '/storage/:path*', destination: `${normalized}/storage/:path*` },
        { source: '/sanctum/:path*', destination: `${normalized}/sanctum/:path*` },
        { source: '/api/:path*', destination: `${normalized}/api/:path*` }
      );
    } else {
      // build log so you can see why rewrites were skipped
      // eslint-disable-next-line no-console
      console.warn('NEXT_PUBLIC_API_URL not set or invalid — skipping remote rewrites');
    }
